...
exports.Activate = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      email: email,
      emailToken: code,
      emailTokenExpires: { $gt: Date.now() }, //to check if the code is expired
    });
    if (!user) {
 ...
 try {
   ...
   if (!user) {
     return res.send({
       success: true,
       message:
         "If that email address is in our database, we will send you an mail to reset your password",
     });
   }
   ...
   user.resetPasswordExpires = expiry; //only 15 minutes
   await user.save();
   return res.send({
     success: true,
     message:
